...
function removeFirstOccurrences(str, value) {
  return str.slice(0, str.indexOf(value)).trim()
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    for (let j = 0; j < width; j += 1) {
      if (j === 0 && i === 0) result += '┌';
      else if (i === 0 && j > 0 && j < width - 1) result += '─';
      else if (i === height - 1 && j > 0 && j < width - 1) result += '─';
...
function encodeToRot13(str) {
  ...
  return str.split('').map((char) => {
    if (!(char in firstHalf || char in secondHalf)) return char;
    return char in firstHalf ? firstHalf[char] : secondHalf[char];
  }).join('');
}
