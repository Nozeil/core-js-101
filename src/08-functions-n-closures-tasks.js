...
function getPolynom(...rest) {
  let counter = rest.length;
  if (!(rest.length)) return null;
  return (x) => rest.reduce((prev, curr, index) => {
    counter -= 1;
    const result = prev + curr * x ** counter;
    if (index === rest.length - 1) counter = rest.length;
    return result;
  }, 0);
}
...
function logger(func, logFunc) {
  return (...rest) => {
    let stringRest = JSON.stringify(rest);
    stringRest = stringRest.slice(1, stringRest.length - 1);
    logFunc(`${func.name}(${stringRest}) starts`);
    const result = func(...rest);
    logFunc(`${func.name}(${stringRest}) ends`);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let copyStartFrom = startFrom;
  return () => {
    const res = copyStartFrom;
    copyStartFrom += 1;
    return res;
  };
}
